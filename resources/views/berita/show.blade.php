                ...
                <article class="mb-0 text-center">
                    ...
                    <header class="mb-4">
                        ...
                        <div class="text-muted fst-italic mb-2">
                            {{ $post->user->name }} | {{ $post->created_at->translatedFormat('l, d F Y') }}
                        </div>
                        <!-- Post categories-->
                        @if ($post->tags()->exists())
                        @foreach ($post->tags as $tag)
                        <a class="badge bg-dark bg-gradient rounded-pill mb-2" href="#!">{{ $tag->name }}</a>
                        @endforeach

                        @endif
                    </header>
                    <figure class="mb-2"><img class="img-fluid rounded" src="{{ asset('storage/'.$post->post_image) }}" alt="..." /></figure>
...
<section class="py-5 bg-light">
    <div class="container">
        <div class="section-title-wrap mb-5">
            <h4 class="section-title">Berita Lain</h4>
        </div>
        <div class="row gx-0">
            <div class="col-lg-8">
